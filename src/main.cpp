...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--list-sources") { whisper_dictate::list_sources(); return 0; }
        if (arg == "--self-test-config") { return whisper_dictate::self_test_config(std::cout) ? 0 : 1; }
    ...
    if (test_mic_seconds > 0) {
        auto samples = whisper_dictate::record_seconds(cfg.input_source, test_mic_seconds);
        auto stats = whisper_dictate::compute_audio_stats(samples);
        std::cout << "sample_count=" << stats.sample_count << " peak=" << stats.peak << " rms=" << stats.rms << "\n";
        return 0;
    }
    if (!test_output.empty()) {
        whisper_dictate::emit_output_text(test_output, cfg.output_mode);
        return 0;
    }
    ...
    auto on_press = [&]() {
        ...
        rec_thread = std::thread([&]() {
            while (recording.load()) {
                try {
                    auto chunk = whisper_dictate::record_seconds(cfg.input_source, 1);
    ...
    auto on_release = [&]() {
        ...
        std::lock_guard<std::mutex> lock(m);
        auto stats = whisper_dictate::compute_audio_stats(utterance);
        std::cout << "utterance complete: samples=" << stats.sample_count << " peak=" << stats.peak << " rms=" << stats.rms << "\n";
    };

    return whisper_dictate::run_hotkey_loop(cfg.hotkey, on_press, on_release);
}
